...
Read `config.md` — contains TELEGRAM_BOT_TOKEN. Parse the token value.
...
Send this email via Gmail connector:
...
**Telegram push for score 80+ jobs only:**
For each job with score >= 80, run this curl command:
```bash
TELEGRAM_TOKEN="[VALUE FROM config.md]"
curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage" \
  -d "chat_id=8764421559" \
  -d "parse_mode=HTML" \
  -d "text=🎯 <b>[SCORE]/100 — [JOB TITLE]</b>%0A[COMPANY] · [LOCATION]%0A✅ [FIT_REASON]%0A⚠️ [GAP]%0A%0A<a href='[URL]'>APPLY NOW →</a>"
```
